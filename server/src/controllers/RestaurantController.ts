...
export default class RestaurantController implements Crud {
    ...
    async create(request: Request, response: Response){
        const {name, adress, food} = request.body;

        const isAnyUndefined = Citi.areValuesUndefined(name, adress, food);
        if(isAnyUndefined) return response.status(400).send();

        const newRestaurant = {name, adress, food};
    ...
    async update(request: Request, response: Response){
        ...
        const isAnyUndefined = Citi.areValuesUndefined(name, adress, food, id);
        if(isAnyUndefined) return response.status(400).send();

        const restaurantWithUpdateValues = { name, adress, food };

        const { httpStatus, messageFromUpdate } = await Citi.updateValue(Restaurant, id, restaurantWithUpdateValues);
